...
const CARRIER_LOGIN_URL = env('CARRIER_LOGIN_URL_AMERICO', '') || env('CARRIER_LOGIN_URL', 'https://portal.americoagent.com/');
...
async function main() {
  ...
  console.log('Opening Americo login:', CARRIER_LOGIN_URL);
  try {
    await page.goto(CARRIER_LOGIN_URL, { waitUntil: 'domcontentloaded', timeout: 120000 });
  } catch (err) {
  ...
  console.log('\nLog in on the Americo portal (portal.americoagent.com). Once you can see the');
  console.log('dashboard / policies pages, press ENTER here to close (session is saved to the');
  console.log('local profile at ' + PROFILE_DIR + ').');
