/**
 * Deal Tracker processing for AHL.
 * Policy and commission table layout matches Aetna/AFLAC.
 */
  ...
  for (const policy of policies) {
    ...
    if (commission && commission.commissionamount != null) {
      ...
      if (parsed != null && parsed > 0) {
        dealValue = parsed
        ccValue = calculateCcValue(parsed, existing?.deal_creation_date ?? (policy.apprecddate || policy.issuedate || null))
      } else if (existing) {
        dealValue =
    ...
    const entry: DealTrackerPreviewEntry = {
      ...
      source_policy_table: 'ahl_policies',
      source_policy_id: policy.id,
      source_commission_table: commission ? 'ahl_commissions' : null,
